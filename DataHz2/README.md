...
- Aggregation：`http://127.0.0.1:5080/aggregation/`
...
- 新增 `src/DataHz.Api/wwwroot/aggregation/index.html`，提供聚焦目录汇总的专用图形界面。
- 新增 `src/DataHz.Api/wwwroot/index.html`，作为 GUI 首页入口（链接 Workbench/Monitor/Swagger/Health）。
- 新增 `/workbench -> /workbench/`、`/aggregation -> /aggregation/` 重定向，统一入口体验。
- 新增 `GET /api/fs/entries` 只读文件系统浏览接口，供前端选择模板文件、源数据目录、输出目录与区划代码文件。
...
- Aggregation 专页增强：目录/文件选择器、选择器路径直达（支持回车跳转）、文件路径自动折算到所在目录、参数即时校验、计划预览、开始汇总按钮、作业轮询进度条、结果统计卡片、日志与错误展示。

### 如何使用与验证

1. 启动 API 服务后访问 `http://127.0.0.1:5080/`，点击 `Task Workbench`。
2. 若是常规目录汇总场景，可直接点击 `Directory Aggregation` 进入专页，并使用“选择”按钮从服务端目录树中选择模板文件、源数据目录、输出目录与区划代码文件。
3. 若目录层级较深，可在 Aggregation 选择器中直接粘贴服务端目录路径后按回车跳转；若粘贴的是模板文件或区划代码文件路径，选择器会自动定位到该文件所在目录。
4. Aggregation 专页点击 `校验参数` 会先调用 `POST /api/tasks/plan` 预览计划；点击 `开始汇总` 会在预览成功后自动调用 `POST /api/jobs/submit`，随后轮询 `GET /api/jobs/{id}` 展示队列状态、执行进度与结果摘要。
5. 在 Workbench 中填写 `TemplatePath/SourceDirectory/TargetDirectory/AreaCodePath` 与索引范围，按顺序执行：
   - `解析模板`（`POST /api/templates/parse`）
   - `生成计划`（`POST /api/tasks/plan`）
   - `同步执行`（`POST /api/tasks/execute`）
6. 点击 `提交异步作业`（`POST /api/jobs/submit`），随后在下方 `Async Jobs` 列表查看状态并点击行加载详情。
7. 若需要中止任务，选中作业后点击 `取消选中作业`（`POST /api/jobs/{id}/cancel`）。
8. 可以先用 `校验参数` 检查输入完整性，或使用 `一键流程` 一次完成解析、计划与异步提交。
9. 可通过 `导出参数/导入参数` 复用任务配置；作业区支持自动刷新（5 秒）与实时队列统计。
10. 作业筛选条件（关键词、状态、take、自动刷新开关）会自动保存在浏览器本地，刷新页面后可恢复。
11. 在作业区可按 `status` 与关键词筛选历史任务，可按状态快速勾选，再批量取消。
12. 批量取消支持并发度设置（1-20）；可启用“跳过已终态”以避免无效取消请求。
13. 若有失败项，可点击 `重试失败取消` 执行一键重试，或点击 `复制失败作业ID` 输出失败清单。
14. 上次批量取消失败会显示原因分组摘要，可点击某个原因一键筛选回作业表（并支持清除原因筛选）。
15. 支持下载失败清单（TXT/JSON）与“清空失败历史”。
16. 选中任一作业后，可点击 `回填选中作业参数` 将该作业请求参数写回计划区；也可点击 `克隆提交选中作业` 快速提交副本任务。
17. 选中作业后可点击 `复制模板路径` 或 `复制路径集`，快速复用路径到外部工具或脚本。
18. 选中作业后可点击 `复制选中作业JSON` 或 `下载选中作业JSON`，用于留档与排障。
19. 作业区支持排序字段/方向设置，并可一键定位当前结果中的最新 failed 作业。
20. 计划区参数可保存为“计划预设”，支持一键加载、重命名、删除、导入/导出 JSON 与一键清空。
21. 计划预设支持“设为默认”，页面初始化时会自动应用默认计划预设（若当前未锁定有效活动预设）。
22. 计划预设支持“置顶常用”，并可在常用区/非常用区内执行上移、下移排序。
23. 计划预设可在排序面板中直接拖拽排序，也可用 `Alt+↑/Alt+↓` 快速调整当前选中预设顺序。
24. 若误操作排序，可点击 `撤销排序`/`重做排序`，或使用 `Alt+Z` 撤销、`Alt+Shift+Z`（或 `Alt+Y`）重做；系统最多保留最近 30 步排序历史，历史列表会展示时间戳与操作摘要，并支持按步点击回退或前进，且刷新页面后仍可继续使用这些历史。
25. 排序历史支持 `导出排序历史` 与 `导入排序历史`（JSON），便于跨浏览器备份和迁移；导入后会自动按当前已有计划预设做清洗对齐。
26. 作业筛选条件可保存为“筛选预设”，支持后续一键加载、删除、导入/导出 JSON 与一键清空。
27. 可将当前筛选结果导出为 CSV 或 JSONL，供离线核对与二次处理。
28. 任一步骤失败时，可在右侧 `Response Console` 查看请求与响应内容用于排错。
