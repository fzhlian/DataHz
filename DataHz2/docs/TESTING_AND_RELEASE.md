...
## 2026-03-05 camelCase suffix redaction hardening

What changed:
- `smoke-test-api.ps1` now redacts camelCase suffix variants for sensitive key families (for example `apiKeyAlt`, `sessionIdBackup`) in both key-value diagnostics and URL query parameters.
- `check-deploy-guards.ps1` extends `smoke-detail-redacts-secrets` with camelCase suffix fields in mock payload and echoed `target_url`, then asserts those raw values never appear in stdout/stderr/report.
- `smoke-baseurl-query-secrets-redacted` now injects camelCase suffix query keys and requires `report.baseUrl` to keep keys while redacting values.
...
## 2026-03-05 URL-encoded bracket-suffix key redaction hardening

What changed:
- `smoke-test-api.ps1` now also redacts sensitive keys when bracket suffixes are URL-encoded (for example `api_key%5B0%5D`, `sessionIdBackup%5Bmeta%5D`) in both key-value diagnostics and URL query text.
- `check-deploy-guards.ps1` extends `smoke-detail-redacts-secrets` and `smoke-baseurl-query-secrets-redacted` with URL-encoded bracket-suffix keys, then asserts raw values never appear in report/stdout/stderr.

How to validate:
- Run: `./DataHz2/scripts/check-deploy-guards.ps1 -PackageZip ./DataHz2/artifacts/packages/datahz2-api-win-x64.zip`
- Expect `smoke-detail-redacts-secrets` and `smoke-baseurl-query-secrets-redacted` to pass with no raw URL-encoded bracket-suffix secret fragments in outputs.

## 2026-03-05 fragment URL-encoded bracket key coverage hardening

What changed:
- `check-deploy-guards.ps1` extends `smoke-detail-redacts-secrets` with fragment parameters using URL-encoded bracket-suffix keys (for example `#...&api_key%5B0%5D=...&sessionIdBackup%5Bmeta%5D=...`) and asserts raw values never appear.
- `smoke-baseurl-query-secrets-redacted` now injects the same fragment URL-encoded bracket-suffix keys in `BaseUrl` and requires `report.baseUrl` to preserve keys while redacting values.
...
## 2026-03-05 double-encoded bracket key redaction hardening

What changed:
- `smoke-test-api.ps1` now also redacts sensitive keys when bracket suffixes are double URL-encoded (for example `api_key%255B0%255D`, `sessionIdBackup%255Bmeta%255D`) in key-value diagnostics and URL query/fragment text.
- `check-deploy-guards.ps1` extends `smoke-detail-redacts-secrets` with double-encoded bracket keys (including nested forms) in payload and echoed `target_url`, and asserts raw values never appear.
- `smoke-baseurl-query-secrets-redacted` now injects the same double-encoded keys in query and fragment sections and validates `report.baseUrl` preserves keys while redacting values.
...
## 2026-03-05 smoke-detail target_url lowercase-hex parity hardening

What changed:
- `check-deploy-guards.ps1` extends the `smoke-detail-redacts-secrets` `target_url` fixtures (both success and error payload branches) with lowercase-hex bracket variants in query and fragment sections: `%5b...%5d`, `%255b...%255d`, and `%25255b...%25255d`.
- `smoke-detail-redacts-secrets` forbidden-secret assertions now include these new lowercase-hex `guard-url-*` and `guard-url-fragment-*` values, so any leakage in detail/stdout/stderr fails the guard case.
...
How to validate:
- Run: `./DataHz2/scripts/check-deploy-guards.ps1 -PackageZip ./DataHz2/artifacts/packages/datahz2-api-win-x64.zip`
- Expect `smoke-detail-redacts-secrets` to pass with no raw `guard-single-quote-*` or `guard-escaped-unicode-quote-*` values in report/stdout/stderr outputs.
